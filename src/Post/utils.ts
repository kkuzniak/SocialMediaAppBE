...
export const postImageFilter = (
  request: Request,
  file: Express.Multer.File,
  callback: FileFilterCallback,
) => {
  const extension = POST_IMAGE_REQUIRED_FILE_TYPES.test(
  ...
  if (!extension || !mimetype) {
    callback(new ApiError(THE_FILE_IS_NOT_AN_IMAGE, 400));

    return;
  }

  if (fileSize > MAX_FILE_SIZE_IN_BYTES) {
    callback(new ApiError(MAXIMUM_SIZE_OF_THE_FILE, 400));
